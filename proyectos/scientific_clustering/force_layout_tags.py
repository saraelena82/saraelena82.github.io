...
zooming_out = 0
circle_diameter = 30			# the diameter of the node-circles  POR LO PRONTO DEJEMOSLO ASI
timestep = 0
...
show_energie_in_background = 0
show_textinformation_in_background = 0
...
# Class for Nodes
class Node:
	def __init__(self, node_id, font_size):
		...
		self.width=0
		self.height=0
	def setWidth(self,mwidth):
		self.width=mwidth
	def setHeight(self,mheight):
		self.height=mheight
...
class Graph:
	...
	def printData(self):
		...
		cloud_filename=sys.argv[3] + "_cloud.dat"
		FILE=open(cloud_filename,"w")
		print 'graph with', len(self.nodes), 'nodes and', len(self.edges), 'edges'
		print
		for node in self.nodes:
                        cx1 = ((node.coordinate_x*scaling_factor + (center_distance/2)) / center_distance * c_width) + border
                	cy1 = ((node.coordinate_y*scaling_factor + (center_distance/2)) / center_distance * c_height) + border
                        #Esto era para ver la esquina superior izquierda de las etiquetas...
                        #c.create_rectangle(cx1, cy1, cx1 + 5, cy1 + 5,fill="#FF0000")
			#print 'x coordinate of', node.id, 'is', node.coordinate_x
			#print 'y coordinate of', node.id, 'is', node.coordinate_y
			#print node.id,' ',node.fontsize,' ', cx1,' ',cx1 + node.width,' ',cy1,' ',cy1 + node.height
			#print
			FILE.write(str(node.id))
			FILE.write(" ")
			FILE.write(str(node.fontsize))
			FILE.write(" ")
			FILE.write(str(cx1))
			FILE.write(" ")
			FILE.write(str(cx1+node.width))
			FILE.write(" ")
			FILE.write(str(cy1))
			FILE.write(" ")
			FILE.write(str(cy1+node.height))
			FILE.write("\n")
		FILE.close()
		c.quit()
	...
	def paintGraph(self):
		...
		for node in g.getNodes():
			...
			for neighbor_id in node.neighbour_ids:
				...
				if (node.id > node2.id):
					...
					c.create_line (x0 + circle_diameter*scaling_factor / 2, y0 + circle_diameter*scaling_factor / 2, x1 + circle_diameter*scaling_factor / 2, y1 + circle_diameter*scaling_factor / 2,fill="#000000", width=0.3)
		...
		for node in g.getNodes():
			...
			#Sara --> PUT TEXT FIRST FOR CALCULATING DIMENSIONS
			fsize=node.fontsize
			
			myFont = tkFont.Font(family="Arial", size=fsize, weight=tkFont.BOLD)
			label=str(node.id)
			(wi,he)=(myFont.measure(label),myFont.metrics("linespace"))
			...
			if (node.cc_number <= 5):
				...
				if node.movable == 1:
                                        c.create_rectangle(x0, y0, x0 + wi*scaling_factor, y0 + he*scaling_factor, fill="",outline="")
				else:
					c.create_oval(x0, y0, x0 + wi*scaling_factor + 10, y0 + he*scaling_factor + 10, fill="",outline="")
			else:
				if (node.cc_number == 6):
					fill_color = "FF2"	# yellow
				if (node.cc_number == 7):
			...
			c.create_text(x0, y0, anchor=tk.NW, text=label, fill="#000000", font= ("Arial", node.fontsize))
	...
	def calculateStep(self):
		...
		eweight=1 #Error extrano en el codigo...
		for node in self.nodes:
			node.force_coulomb = [0,0]
			for node2 in self.nodes:
                                #modificacion por sara
                                if node2.id in node.neighbour_ids:
                                        mykey=node2.id,"_",node.id
                                        ikey=node.id,"_",node2.id
                                        if mykey in edge_weights:
                                                eweight=edge_weights[node2.id,"_",node.id]
                                        elif ikey in edge_weights:
                                                eweight=edge_weights[node.id,"_",node2.id]
                                        else:
                                                eweight=1
				if (node.id != node2.id) and (node.cc_number == node2.cc_number):
					#distance_x = (node.coordinate_x - node2.coordinate_x)*(1/float(eweight)) 
					#distance_y = (node.coordinate_y - node2.coordinate_y)*(1/float(eweight))
                                        cx1 = ((node.coordinate_x*scaling_factor + (center_distance/2)) / center_distance * c_width) + border
                                        cx2 = ((node2.coordinate_x*scaling_factor + (center_distance/2)) / center_distance * c_width) + border
                			cy1 = ((node.coordinate_y*scaling_factor + (center_distance/2)) / center_distance * c_height) + border
                			cy2 = ((node2.coordinate_y*scaling_factor + (center_distance/2)) / center_distance * c_height) + border
                                        if cx1 > cx2:
                                                        cx1+=node.width
                                        else:
                                                        cx2+=node2.width
                                        ...
                                        #c.create_rectangle(cx1, cy1, cx1 + 5, cy1 + 5,fill="#FF0000")
                                        #c.create_rectangle(cx2, cy2, cx2 + 5, cy2 + 5,fill="#00FF00")

                                        nx1 = ((((cx1 - border) / c_width) * center_distance) - (center_distance/2))/scaling_factor
                                        nx2 = ((((cx2 - border) / c_width) * center_distance) - (center_distance/2))/scaling_factor
                                        ny1 = ((((cy1 - border) / c_width) * center_distance) - (center_distance/2))/scaling_factor
                                        ny2 = ((((cy2 - border) / c_width) * center_distance) - (center_distance/2))/scaling_factor

                                        distance_x = (nx1 - nx2)*(1/float(eweight))
                                        distance_y = (ny1 - ny2)*(1/float(eweight))
		...
		for node in self.nodes:
			...
			for neighbor_id in node.neighbour_ids:
				...
				if cx1 < cx2:
                                ...
                                #c.create_rectangle(100, 100, 25, 25,fill="#0000FF")
                                #c.create_rectangle(cx2, cy2, cx2 + 5, cy2 + 5,fill="#00FFFF")
                                ...
                                distance_x = (nx1 - nx2)*(float(eweight))
                                distance_y = (ny1 - ny2)*(float(eweight))
				radius = sqrt(distance_x*distance_x + distance_y*distance_y)
				if radius != 0:
...
print
print 'reading nodes file', nodes_filename,
try:
        nf = open(nodes_filename, 'r')
        nodes_rows = nf.readlines()
        nf.close()
except IOError:
        print nodes_filename, 'could not be opened'
        sys.exit(1)
...
#agregar nodos
for line in nodes_rows:
        new_line = line.strip()
        line_array = new_line.split()
        if len(line_array) == 2:
		(node_1, font_size) = line_array
		g.addNode(node_1,  font_size)
...
g.printData()
c.update()
c.postscript(file=sys.argv[1]+"_cloud.eps")
#g.printEdges()

#c.mainloop()


## Este algoritmo coloca el grafo en un plano con cuatro cuadrantes. Habria que hacer una funcion que mapeara las coordenadas al espacio manejado por Xfig
